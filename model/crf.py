...
class CRF(nn.Module):
    ...
    def _viterbi_decode(self, feats, mask):
        ...
        for i, history in enumerate(partition_history):
            print(f"{i}, {history.size()}")

        partition_history = torch.cat(partition_history, 0)
        partition_history = partition_history.view(seq_len, batch_size, -1).transpose(1,0).contiguous() ## (batch_size, seq_len. tag_size)
